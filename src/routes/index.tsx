...
function HomePage() {
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-primary text-primary-foreground">
        ...
        <div className="container-page relative py-20 md:py-32">
          <div className="max-w-3xl animate-fade-up">
            ...
            <p className="mt-5 text-base md:text-lg opacity-85 max-w-2xl leading-relaxed">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                to="/departments"
                className="inline-flex items-center gap-2 rounded-md bg-gold px-5 py-3 text-sm font-semibold text-gold-foreground shadow-soft hover:opacity-95 transition"
              >
                Explore Departments <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                to="/about"
                className="inline-flex items-center gap-2 rounded-md border border-white/25 bg-white/5 px-5 py-3 text-sm font-semibold hover:bg-white/10 transition"
              >
            ...
            <dl className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-6 max-w-2xl">
              {[
                ...
                <div key={l}>
                  <dt className="font-display text-2xl md:text-3xl font-semibold text-gold">{v}</dt>
                  <dd className="text-xs md:text-sm opacity-75 mt-1">{l}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
          {QUICK.map(({ to, icon: Icon, label, desc }) => (
            <Link
              key={label}
              to={to}
              className="group rounded-2xl border border-border bg-card p-5 shadow-soft hover:shadow-elevated hover:-translate-y-0.5 transition"
            >
              <div className="grid h-11 w-11 place-items-center rounded-xl bg-primary/5 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <SectionHeading
              ...
              description="Industry-relevant curriculum, well-equipped labs, 100 Mbps networked classrooms and dedicated mentoring — on a calm 9.74-acre campus adjacent to the hills at Rebaka."
            />
            <ul className="mt-8 grid gap-4 sm:grid-cols-2">
              {[
                ...
                ["Industry-ready labs", "150+ computers, CISCO routing & switching setup"],
                ["Safe & modern campus", "CCTV surveillance, AEBAS biometric, RO/UV drinking water"],
              ].map(([t, d]) => (
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              [9, "+ acres", "Green campus"],
            ].map(([n, s, l]) => (
              <div key={l as string} className="rounded-2xl border border-border bg-card p-6 shadow-soft">
      ...
      <Section muted>
        ...
        <div className="mt-12 grid sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
          {DEPTS.map(({ icon: Icon, name, code, desc }) => (
            <div key={code} className="group rounded-2xl border border-border bg-card p-7 shadow-soft hover:shadow-elevated transition">
              ...
              <Link to="/departments" className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-primary hover:text-gold">
        ...
        <div className="grid lg:grid-cols-[1.2fr_1fr] gap-12 items-center">
          <div>
            <SectionHeading
              ...
              description="Nearly all our graduates progress to higher education — pursuing B.Tech via ECET or AMIE — while interested students are supported by the placement cell for industry roles and apprenticeships."
            />
            ...
            <Link to="/placements" className="mt-8 inline-flex items-center gap-2 rounded-md bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground hover:opacity-95">
              Explore career pathways <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
          <div className="rounded-2xl border border-border bg-surface p-6">
            <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Campus facilities</div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              {["Seminar Hall", "Central Library", "Girls Waiting Hall", "Playgrounds", "RO/UV Drinking Water", "CCTV Surveillance", "AEBAS Biometric", "CISCO Networking", "Skill Dev. Center", "Intercom System"].map((r) => (
                <div key={r} className="rounded-lg bg-card border border-border px-4 py-3 text-sm font-semibold text-foreground text-center shadow-soft">
                  {r}
                </div>
              ))}
            </div>
      ...
      {/* PRINCIPAL */}
      <Section muted>
        <div className="grid lg:grid-cols-[auto_1fr] gap-10 items-center max-w-5xl mx-auto">
          <div className="relative">
            <img
              ...
              height={1024}
              loading="lazy"
              className="h-64 w-64 md:h-80 md:w-80 rounded-2xl object-cover shadow-elevated"
            />
            <div className="absolute -bottom-4 -right-4 rounded-xl bg-gold text-gold-foreground px-4 py-2 text-xs font-semibold shadow-soft">
              Principal's Desk
            </div>
          </div>
          <div>
            <Quote className="h-10 w-10 text-gold" />
            <p className="mt-4 text-lg md:text-xl leading-relaxed text-foreground">
            ...
            <div className="mt-6">
              <div className="font-display font-semibold text-foreground">Prof. I.V.S.S. Srinivasa Rao, M.E.</div>
              <div className="text-sm text-muted-foreground">Principal, Government Polytechnic, Anakapalli</div>
              <Link to="/principal" className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-primary hover:text-gold">
      ...
      <Section>
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-4">
          {NOTICES.map((n) => (
            <article key={n.title} className="group rounded-xl border border-border bg-card p-5 hover:shadow-elevated transition">
              ...
              <button className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-gold">
      ...
      {/* GALLERY */}
      <Section muted>
        <SectionHeading eyebrow="Campus Life" title="A glimpse of Government Polytechnic, Anakapalli" center />
        <div className="mt-12 grid grid-cols-2 md:grid-cols-3 gap-3">
          {GALLERY.map((g, i) => (
            <div
              key={i}
              className={`overflow-hidden rounded-xl ${i === 0 ? "md:col-span-2 md:row-span-2 aspect-square md:aspect-auto" : "aspect-[4/3]"}`}
            >
              <img
                src={g.src}
                alt={g.alt}
        ...
        <div className="mt-8 text-center">
          <Link to="/gallery" className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-5 py-3 text-sm font-semibold hover:bg-accent">
      ...
      <section className="bg-primary text-primary-foreground">
        <div className="container-page py-16 md:py-20 text-center">
          <h2 className="font-display text-3xl md:text-4xl font-semibold">Have a question? We're here to help.</h2>
          <p className="mt-3 max-w-xl mx-auto opacity-85">
          ...
          <Link to="/contact" className="mt-8 inline-flex items-center gap-2 rounded-md bg-gold px-6 py-3 text-sm font-semibold text-gold-foreground hover:opacity-95">
